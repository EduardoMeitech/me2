...
**IMPORTANT — Production bar colors:** Green = hour met or exceeded target (`3600 / standard_cycle_time_s`). Orange = hour below target. Target is P75 of real production data — set via `standard_cycle_time_s` in `me2_equipment`.

**IMPORTANT — StatusTimeline vs StatusPareto consistency:** Both use the same `statusEvents` data with `duration_min` from the API. The StatusTimeline positions each segment by its **actual timestamp** within the shift (not concatenated), so visual proportions match the Pareto percentages. The status endpoint caps the last event's duration at `min(shift_end, now)` to avoid inflating historical data.
